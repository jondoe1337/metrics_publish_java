...
/**
 * Provisional API which is subject to change. Represents a request to the New Relic metrics API.
 *
 * <p>
 * A {@code Request} has an associated {@link Context} and a list of metrics for a given component. The {@code Request} is sent in the JSON format.
 *
 */
public class Request
{
     ...
     * <p>
     * See also {@link Context#createRequest()}
     *
     * @param context
     *            the {@link Context} for the {@code Request}
     */
    ...
    /**
     * Add metric to the {@code Request} for a given component. The {@link Number} value is converted to a {@code float}. The count is assumed to be
     * 1, while minValue and maxValue are set to value. Sum of squares is calculated as the value squared.
     *
     * @param component
     *            the {@code ComponentData} the metric should be added to
     * @param name
     *            the name of the metric
     * @param value
     *            the Number value for the metric
    ...
    /**
     * Add metric to the {@code Request} for a given component. All {@link Number} values are converted to {@code floats}.
     *
     * @param component
     *            the {@code ComponentData} the metric should be added to
     * @param name
     ...
     *            the Number value for the metric
     * @param minValue
     *            the minimum Number value for the metric
     * @param maxValue
     *            the maximum Number value for the metric
     * @param sumOfSquares
     *            the sum of squared values for the metric
     * @return MetricData the newly added metric
     */
    ...
    private void processResponse(HttpURLConnection connection) throws IOException
    {
        ...
        else
        {
            ...
            else if (isRemotelyDisabled(responseCode, responseBody))
            {
                ...
                logger.error("Agent has been disabled remotely by New Relic");
    ...
    /**
     * Checks if the Collector is currently unavailable. A 503 response code (HTTP_UNAVAILABLE) from the Collector is an unavailable response.
     *
     * @param responseCode
     * @return boolean
     */
    ...
    /**
     * Checks if the agent has been remotely disabled by the Collector. A 403 response code (HTTP_FORBIDDEN) with DISABLE_NEW_RELIC as the response
     * body indicate that the agent should be remotely disabled.
     *
     * @param responseCode
     * @param responseBody
     * @return boolean
    ...
    /**
     * Checks if the Collector response is Ok. A 200 response code (HTTP_OK) with status of "ok" in the response body is an Ok response.
     *
     * @param responseCode
     * @param responseBody
     * @return boolean
    ...
    /**
     * Get an InputStream from the server response. Valid responses have response codes less than 400 (bad request). Valid responses are read from
     * getInputStream(), while error responses must be read from getErrorStream()
     *
     * @param responseCode
     * @param connection
     * @return InputStream
    ...
    /**
     * Get status message by parsing JSON response body. Will return null if no status is present.
     *
     * @param responseBody
